    ...
    if is_own_profile:
        context["account"] = {
            ...
            "profile_pictures": pic_selection(
                target_user
            ),  # should be settings.PPIC_SELECTION
        }
...
def update_profile(request):
    try:
        ...
        if OngoingGame.player_in_game(user_uuid):
            return JsonResponse(
                {"error": "Cannot update profile while in an active game"}, status=400
            )

        if Tournament.player_in_tournament(user_uuid):
            return JsonResponse(
                {"error": "Cannot update profile while in a tournament"}, status=400
            )
        ...
        return JsonResponse(
            {"error": "An error occurred while updating the profile"}, status=500
        )
    ...
    try:
        ...
        if old_profile_pic:
            old_profile_pic_path = os.path.join(
                settings.MEDIA_ROOT,
                old_profile_pic.replace(
                    f"https://{settings.WEB_HOST}{settings.MEDIA_URL}", ""
                ),
            )
            if os.path.exists(old_profile_pic_path) and old_profile_pic_path.startswith(
                user_dir
            ):
