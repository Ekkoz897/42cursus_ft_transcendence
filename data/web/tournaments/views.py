...
from backend.decorators import require_header, jwt_auth_required
...
def tournament_create(request):
    if Tournament.player_in_tournament(
        str(request.user.uuid)
    ):  # check for uuid instead
        return JsonResponse(
            {"status": "error", "message": "You are already in a tournament"},
            status=400,
        )

    tournament_id = generate_tournament_id()
    tournament = Tournament.create_tournament(
        tournament_id=tournament_id, players=[request.user.username]
    )

    return JsonResponse({"status": "created", "tournament_id": tournament_id})
...
def tournament_leave(request):
    ...
    if tournament and tournament.status == "IN_PROGRESS":
        return JsonResponse(
            {"status": "error", "message": "Cant leave a tournament in progress"},
            status=400,
        )

    if not tournament:
        return JsonResponse(
            {"status": "error", "message": "You are not in a tournament"}, status=400
        )
...
def tournament_join(request):
    ...
    if not tournament:
        return JsonResponse(
            {"status": "error", "message": "Tournament not found"}, status=404
        )

    if tournament.status != "REGISTERING":
        return JsonResponse(
            {"status": "error", "message": "Tournament not accepting players"},
            status=400,
        )
...
def tournament_list(request):
    ...
    user_tournament = Tournament.objects.filter(
        player_ids__has_key=user_uuid, status__in=["REGISTERING", "IN_PROGRESS"]
    ).first()
    ...
    if user_tournament:
        ...
        if user_tournament.rounds and user_tournament.current_round < len(
            user_tournament.rounds
        ):
            current_round = user_tournament.rounds[user_tournament.current_round]
            for match in current_round:
                match["is_player_match"] = (
                    match["player1"] == username or match["player2"] == username
                )

    return JsonResponse(
        {
            "in_tournament": bool(user_tournament),
            "current_tournament_id": user_tournament.tournament_id
            if user_tournament
            else None,
...
def user_tournaments(request):
    user_uuid = str(request.user.uuid)
    last_tournament = (
        Tournament.objects.filter(player_ids__has_key=user_uuid, status="COMPLETED")
        .order_by("-updated_at")
        .first()
    )

    if not last_tournament:
        return JsonResponse({"status": "success", "has_history": False})
    ...
    if last_tournament.winner_id != user_uuid:
        for round_num, round_matches in enumerate(last_tournament.rounds):
            for match in round_matches:
                ...
                if (player1_id == user_uuid or player2_id == user_uuid) and match[
                    "status"
                ] == "COMPLETED":
                    winner_id = None
                    for uuid, username in last_tournament.player_ids.items():
